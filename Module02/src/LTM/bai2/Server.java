...
public class Server {
        ...
        public void run() {
            try {
                ...
                while(true)
                {
                    ...
                    int countchar = SOKYTU(TextIn);
                    int countvowel = SONGUYENAM(TextIn);
                    int[] arr = {0,0,0,0,0} ;
                    arr = CountRepeatVowel(TextIn);
                    dos.writeUTF("Chuyển sang chữ thường: " +CHUTHUONG(TextIn)+ "\n" +
                            "Chuyển sang chữ hoa " + CHUHOA(TextIn)+ "\n" +
                            "String after Upper & Lower: " + ToUpper_Lower(TextIn)+ "\n" +
                            "Số kí tự: " + countchar + "\n" +
                            "Số nguyên âm: " + countvowel + "\n" +
    ...
    public static int SONGUYENAM(String s)
    {
        int countvowel = 1;
        for(int i = 0; i< s.length(); i++)
        {
